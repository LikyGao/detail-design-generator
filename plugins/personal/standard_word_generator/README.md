...
v0.0.15では、Word固有のnumbering情報を可視文字より優先し、`numId`から
`abstractNumId`と`ilvl 0～5`を解決します。numbering levelの`numFmt`、`lvlText`、
`pStyle`、indent、symbol fontと、段落styleの`basedOn`継承チェーンも保存します。
具体styleを`basedOn`祖先より先に照合し、`ilvl 0～5`をそれぞれ`level_1`、
`level_2`、`level_4`、`level_3`、`level_5`、`level_6`へ変換します。
これにより同じ「・」を使う別階層やWingdingsの矢印を圧縮せず再生成できます。可視marker判定はnumberingがない場合だけ
fallbackとして使用します。

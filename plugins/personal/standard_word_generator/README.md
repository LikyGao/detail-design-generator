# Standard Word Generator v0.0.23
...
Word/UIの`スタイル0`（内部`paragraph_style=level_1`、native `ilvl=0`）が章節内の
2番目以降の本文グループを開始するとき、その段落自身へ10.5ptの`space_before`を設定します。
Heading直後の最初の本文、および`スタイル1`以降の子階層には追加の段前・段後間隔を
設定しません。テンプレート由来の空段落を読み取ったり、新しい空の`w:p`を挿入したり
せず、生成DOCXの段落プロパティだけでグループ間の視覚的な余白を維持します。
